...
def get_parametric_config():
    """Returns the model config dict for the ParametricAmp architecture."""
    return {
        "preamp": {
            "num_biquad_sections_pre": 2,
            "num_biquad_sections_post": 2,
            "gru_hidden_size": 1,
        },
        "tonestack": {
            "sample_rate": 48000.0,
...
def _get_configs(
    ...
    if model_type == "ParametricAmp":
        # Register the ParametricAmp model if not already registered
        from ..models.parametric.parametric_amp import ParametricAmp as _ParametricAmp
        from .lightning_module import LightningModule as _LM

        try:
            _LM.register_net_initializer(
                "ParametricAmp", _ParametricAmp.init_from_config
            )
        except KeyError:
...
def train(
    ...
    if model_type == "ParametricAmp":
        from ..models.parametric.lightning_module import (
            ParametricLightningModule as _ParametricLM,
        )

        _lightning_cls = _ParametricLM
    else:
        _lightning_cls = _LightningModule
    model = _lightning_cls.init_from_config(model_config)
    ...
    if best_checkpoint != "":
        model = _lightning_cls.load_from_checkpoint(
            trainer.checkpoint_callback.best_model_path,
            **_lightning_cls.parse_config(model_config),
        )
    model.cpu()
    model.eval()
